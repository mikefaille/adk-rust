...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    impl adk_realtime::runner::EventHandler for LogEventHandler {
        async fn on_audio(&self, _audio: &[u8], _item_id: &str) -> adk_realtime::Result<()> { Ok(()) }
        ...
        async fn on_speech_started(&self, _ms: u64) -> adk_realtime::Result<()> { Ok(()) }
        async fn on_speech_stopped(&self, _ms: u64) -> adk_realtime::Result<()> { Ok(()) }
        async fn on_response_done(&self) -> adk_realtime::Result<()> { Ok(()) }
    }
    ...
    let runner = Arc::new(
        RealtimeRunner::builder()
            .model(Arc::new(model))
            .config(RealtimeConfig::default().with_instruction("You are a helpful assistant in a LiveKit room."))
