...
impl<T: EventHandler> EventHandler for LiveKitEventHandler<T> {
    async fn on_audio(&self, audio_base64: &str, item_id: &str) -> Result<()> {
        // 1. Decode base64
        if let Ok(data) = STANDARD.decode(audio_base64) {
            // 2. Convert bytes to i16 (assuming PCM16 LE)
            let i16_samples = bytemuck::cast_slice::<u8, i16>(&data).to_vec();
            
            // 3. Push to LiveKit source
        ...
        // 4. Delegate to inner
        self.inner.on_audio(audio_base64, item_id).await
    }
...
pub fn bridge_input(track: RemoteAudioTrack, runner: Arc<RealtimeRunner>) {
    tokio::spawn(async move {
        let mut reader = livekit::webrtc::audio_stream::native::NativeAudioStream::new(track);
