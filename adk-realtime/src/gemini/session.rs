...
use crate::session::{
    ...
    RecoveryContinuity, RecoveryDisposition, RecoveryOutcome,
};
...
pub struct GeminiRealtimeSession {
    ...
    /// Explicit availability state tracking.
    availability: Arc<ParkingMutex<RealtimeAvailability>>,
}
...
impl GeminiRealtimeSession {
    ...
    pub fn new_for_test(
        ...
        Self {
            ...
            availability: Arc::new(ParkingMutex::new(RealtimeAvailability::Connected)),
        }
    }
...
impl RealtimeSession for GeminiRealtimeSession {
    ...
    fn recovery_disposition(&self, error: &RealtimeError) -> RecoveryDisposition {
        if error.is_connection_reset() {
            ...
            match error {
                RealtimeError::ConnectionError(_) | RealtimeError::MessageError(_) => {
                    RecoveryDisposition::Retryable
                }
    ...
    async fn recover_once(
        ...
        let reconnect_model = self.reconnect_model.clone();

        // 1. Open candidate socket
        let request = reconnect_url.into_client_request().map_err(|e| {
        ...
        tracing::debug!(status = ?response.status(), "GeminiRealtimeSession: candidate handshake complete");

        let (mut candidate_sink, mut candidate_source) = ws_stream.split();

        // 2. Start candidate writer task
        let candidate_cancel = tokio_util::sync::CancellationToken::new();
        let writer_cancel = candidate_cancel.clone();
        let (candidate_tx, mut candidate_rx) = mpsc::channel::<Message>(WRITER_CHANNEL_CAPACITY);

        let writer_connected = Arc::clone(&self.connected);
        let candidate_writer_connected = writer_connected.clone();
        let candidate_writer = tokio::spawn(async move {
            loop {
                tokio::select! {
                    _ = writer_cancel.cancelled() => {
                        tracing::info!("gemini candidate writer task cancelled");
                        break;
                    }
                    msg = candidate_rx.recv() => {
                        match msg {
                            Some(message) => {
                                let should_close = matches!(message, Message::Close(_));
                                if let Err(e) = candidate_sink.send(message).await {
                                    candidate_writer_connected.store(false, Ordering::SeqCst);
                                    tracing::warn!(error = %e, "gemini candidate writer send failed");
                                    break;
                                }
        ...
        // 3. Create a scope-based cleanup guard to ensure candidate gets cleaned up if we fail.
        ...
        let mut cleanup = CandidateCleanup {
            cancel: candidate_cancel.clone(),
            writer: Some(candidate_writer),
            active: true,
        };

        // 4. Send setup as FIRST client frame
        ...
        let realtime_input_config = gemini_realtime_input_config(&config);
        let system_instruction = config.instruction.as_ref().map(|text| GeminiContent {
            parts: vec![GeminiPart { text: Some(text.clone()), inline_data: None }],
        });
        ...
        let setup_msg = GeminiClientMessage {
            setup: Some(GeminiSetup {
                model: Some(reconnect_model.clone()),
                system_instruction,
                generation_config: Some(generation_config),
                tools,
        ...
        let msg_str = serde_json::to_string(&setup_msg)
            .map_err(|e| RealtimeError::protocol(format!("JSON serialize error: {}", e)))?;

        candidate_tx.send(Message::Text(msg_str.into())).await.map_err(|e| {
            RealtimeError::connection(format!("reconnect: failed to queue setup frame: {e}"))
        })?;

        // 5. Wait for setupComplete (translated as SessionCreated)
        let mut setup_complete_received = false;
        let deadline = std::time::Duration::from_secs(5);

        let setup_result = tokio::time::timeout(deadline, async {
            while let Some(msg_res) = candidate_source.next().await {
                match msg_res {
                    Ok(Message::Text(text)) => {
                        let translated =
                            Self::translate_event_logged(&text, Some(&self.frame_log))?;
                        if translated
                            .iter()
                            .any(|e| matches!(e, ServerEvent::SessionCreated { .. }))
                        {
                    ...
                    Ok(Message::Close(close_frame)) => {
                        return Err(RealtimeError::connection(format!(
                            "reconnect: connection closed by server during setup: {:?}",
                            close_frame
                        )));
                    }
                    Ok(_) => {}
                    Err(e) => {
                        return Err(RealtimeError::connection(format!(
                            "reconnect: connection error during setup: {}",
                            e
                        )));
                    }
                }
            ...
            if setup_complete_received {
                ...
                Err(RealtimeError::connection("reconnect: server closed before setupComplete"))
            }
        })
        .await;
        ...
        match setup_result {
            ...
            Err(_) => {
                return Err(RealtimeError::connection(
                    "reconnect: timed out waiting for setupComplete (5s)",
                ));
            }
        }

        // 6. Candidate is Ready -> Publish candidate as the active transport generation
        let published_writer = cleanup.writer.take().unwrap();
        {
        ...
        }
        {
            ...
            if let Some(old_wt) = wt.take() {
                old_wt.abort();
            }
            *wt = Some(published_writer);
        }
        {
