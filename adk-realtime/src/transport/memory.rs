use super::{RealtimeMediaTransport, event::{TransportEvent, TransportControl}};
use crate::{audio::{AudioChunk, AudioFormat}, error::Result};
...
impl InMemoryTransport {
    ...
    pub async fn push_event(&self, event: TransportEvent) -> Result<()> {
        self.event_tx.send(Ok(event)).await.map_err(|_| crate::error::RealtimeError::provider("Channel closed"))
    }
