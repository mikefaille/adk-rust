...
mod tests {
    ...
    async fn test_interrupted_session_store() {
        ...
        // Store the session
        store.store(&SessionId::from("session-789"), session_state.clone()).await;

        // Verify it's stored
        assert!(store.is_interrupted(&SessionId::from("session-789")).await);

        // Get the session
        let retrieved = store.get(&SessionId::from("session-789")).await;
        ...
        // Remove the session
        let removed = store.remove(&SessionId::from("session-789")).await;
    ...
    async fn test_graph_interrupt_handler() {
        ...
        assert!(handler.is_interrupted(&SessionId::from("session-123")).await);

        let interrupted_state =
            handler.get_interrupted_state(&SessionId::from("session-123")).await;
        ...
        // Clear the interrupt
        let cleared = handler.clear_interrupted_state(&SessionId::from("session-123")).await;
    ...
    async fn test_cleanup_old_sessions() {
        ...
        store.store(&SessionId::from("old-session"), session_state).await;
        ...
        store.store(&SessionId::from("recent-session"), session_state).await;

        // Cleanup sessions older than 1 hour
        store.cleanup_old(60 * 60 * 1000).await;

        // Old session should be removed
        assert!(!store.is_interrupted(&SessionId::from("old-session")).await);
        // Recent session should still exist
        assert!(store.is_interrupted(&SessionId::from("recent-session")).await);
    }
}
