...
fn user_profile_template(data: &TemplateData) -> Vec<Component> {
    ...
    let name = user.map(|u| u.name.clone()).unwrap_or_else(|| "User".to_string());
    let email = user.map(|u| u.email.clone()).unwrap_or_else(|| "user@example.com".to_string());
    let role = user.and_then(|u| u.role.clone()).unwrap_or_else(|| "Member".to_string());

    vec![Component::Card(Card {
        id: Some("profile-card".to_string()),
        title: Some(data.title.clone().unwrap_or_else(|| "User Profile".to_string())),
        description: None,
        content: vec![
            Component::Text(Text {
                id: None,
...
fn status_dashboard_template(data: &TemplateData) -> Vec<Component> {
    ...
    vec![
        Component::Text(Text {
            id: None,
            content: data.title.clone().unwrap_or_else(|| "System Status".to_string()),
...
mod tests {
    ...
    fn test_template_from_name() {
        assert_eq!(UiTemplate::from_name("registration"), Some(UiTemplate::Registration));
        assert_eq!(UiTemplate::from_name("signup"), Some(UiTemplate::Registration));
