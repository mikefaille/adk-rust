...
impl VertexBackend {
    ...
    where
        ...
        U: serde::de::DeserializeOwned,
    {
        let response_value = serde_json::to_value(&res).context(GoogleCloudResponseSerializeSnafu)?;
        serde_json::from_value(response_value).context(GoogleCloudResponseDeserializeSnafu)
    }
}
...
fn build_vertex_prediction_service(
    ...
    Error,
> {
    let build_in_runtime = |endpoint: String, auth: GoogleCloudAuth| -> Result<
        (
            PredictionService,
            JobService,
            GenAiCacheService,
        ...
        runtime.block_on(async {
            let credentials = match auth {
                GoogleCloudAuth::ApiKey(api_key) => {
                    credentials::api_key_credentials::Builder::new(api_key)
                        .build()
                }
                GoogleCloudAuth::Adc => {
    ...
    if tokio::runtime::Handle::try_current().is_ok() {
        ...
        return worker
            .join()
            .map_err(|_| Error::GoogleCloudInitThreadPanicked)?;
    }
