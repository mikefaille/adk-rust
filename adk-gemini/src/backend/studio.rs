...
impl StudioBackend {
    ...
    async fn get_json<T: serde::de::DeserializeOwned>(&self, url: Url) -> Result<T, Error> {
        self.perform_request(|c| c.get(url), |r| async move { r.json().await.context(DecodeResponseSnafu) }).await
    }
    ...
    async fn post_json<T: serde::de::DeserializeOwned, B: Serialize + ?Sized>(
        ...
        self.perform_request(|c| c.post(url).json(body), |r| async move { r.json().await.context(DecodeResponseSnafu) }).await
    }
    ...
    async fn create_upload(&self, size: usize, display_name: Option<String>, mime_type: Mime) -> Result<Url, Error> {
         let url = self.base_url.join("upload/v1beta/files").context(ConstructUrlSnafu { suffix: "upload/v1beta/files".to_string() })?;
         ...
         headers.insert("X-Goog-Upload-Header-Content-Type", HeaderValue::from_str(mime_type.as_ref()).unwrap());
         ...
         let upload_url = response.headers().get("X-Goog-Upload-URL")
             .ok_or_else(|| Error::MissingResponseHeader { header: "X-Goog-Upload-URL".to_string() })?
             .to_str()
             .map_err(|_| Error::MissingResponseHeader { header: "X-Goog-Upload-URL (invalid utf8)".to_string() })?;
...
impl GeminiBackend for StudioBackend {
    ...
    async fn upload_file(
        ...
        let upload_response: UploadResponse = self.perform_request(
            |c| {
                c.post(upload_url)
    ...
    async fn download_file(&self, name: &str) -> Result<Vec<u8>, Error> {
        let mut url = self.base_url.join(&format!("/download/v1beta/{name}:download")).context(ConstructUrlSnafu { suffix: format!("/download/v1beta/{name}:download") })?;
