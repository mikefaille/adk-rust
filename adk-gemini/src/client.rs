...
pub enum Error {
    ...
    #[cfg(feature = "vertex")]
    #[snafu(display("failed to fetch google cloud credentials"))]
    GoogleCloudCredentialsFetch {
    ...
    #[cfg(any(feature = "vertex", feature = "studio"))]
...
/// Configuration for the Gemini Live backend.
#[derive(Debug, Clone)]
pub enum GeminiLiveBackend {
    ...
    /// Vertex AI using pre-fetched OAuth credentials.
    #[cfg(feature = "vertex")]
    Vertex(VertexContext),
    /// Vertex AI using Application Default Credentials (auto-fetches token).
    #[cfg(feature = "vertex")]
    VertexADC {
...
impl GeminiClient {
    ...
    fn with_vertex<M: Into<Model>>(
        ...
        Self {
            model: model.into(),
            backend: GeminiBackend::Vertex(VertexClient {
                prediction,
                credentials,
                project_id,
                location,
...
///     .with_model(Model::from(Model::GEMINI_2_5_PRO))
///     .build()?;
/// # Ok(())
/// # }
/// ```
...
impl GeminiBuilder {
    ...
    /// Creates a builder for Vertex AI with Application Default Credentials.
    ///
    /// This is the recommended way to create a client for Vertex AI, as it
    /// automatically handles authentication using ADC (Application Default Credentials).
    ///
    /// # Example
    /// ```rust,no_run
    /// use adk_gemini::GeminiBuilder;
    ///
    /// let client = GeminiBuilder::for_vertex("my-project", "us-central1")?
    ///     .with_model("gemini-3-flash-preview")
    ///     .build()?;
    /// # Ok::<(), adk_gemini::ClientError>(())
    /// ```
    #[cfg(feature = "vertex")]
    pub fn for_vertex<P, L>(project: P, location: L) -> Result<Self, Error>
    where
        P: Into<String>,
        L: Into<String>,
    {
        ...
        Ok(Self {
            ...
            base_url: DEFAULT_BASE_URL.clone(),
            api_key: None,
            google_cloud: Some(GoogleCloudConfig {
                project_id: project.into(),
                location: location.into(),
            }),
            google_cloud_auth: Some(GoogleCloudAuth::Credentials(credentials)),
        })
    }
    ...
    pub fn build(self) -> Result<Gemini, Error> {
        ...
        if let Some(config) = &self.google_cloud {
            let model =
                Model::new(self.model.vertex_model_path(&config.project_id, &config.location));
...
impl Gemini {
    ...
    /// Create a new client for the Gemini 2.5 Pro model using Vertex AI (Google Cloud) endpoints.
    ///
    /// Note: Authentication uses API keys or service accounts.
    #[cfg(feature = "vertex")]
    pub fn vertex_pro<K: AsRef<str>, P: AsRef<str>, L: AsRef<str>>(
        api_key: K,
        project_id: P,
        location: L,
    ) -> Result<Self, Error> {
