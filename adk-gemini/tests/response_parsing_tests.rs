use adk_gemini::{
    BlockReason, FinishReason, FunctionCall, GenerationResponse, HarmCategory,
    HarmProbability, Modality, Model, Part, Role,
};
...
fn test_text_with_thought_signature() {
    ...
    let (first_text, is_thought, thought_sig): (String, bool, Option<&String>) = text_with_thoughts[0].clone();
    assert!(is_thought);
    assert!(thought_sig.is_none());
    assert!(first_text.contains("here's what I'm thinking"));

    let (second_text, is_thought, thought_sig): (String, bool, Option<&String>) = text_with_thoughts[1].clone();
    assert!(!is_thought);
