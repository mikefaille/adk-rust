...
fn test_context_creation() {
    ...
    assert_eq!(ctx.user_content().role, adk_core::types::Role::User);
}
...
fn conversation_history_preserves_tool_role() {
    ...
    assert_eq!(history[0].role, adk_core::types::Role::User);
    assert_eq!(history[1].role, adk_core::types::Role::Model);
    assert_eq!(history[2].role, adk_core::types::Role::Tool); // NOT "model"
}
...
fn conversation_history_maps_agent_events_to_model() {
    ...
    let history = mutable.conversation_history();
    assert_eq!(history[0].role, adk_core::types::Role::Model);
}
