...
pub fn content_to_chat_message(content: &Content) -> Option<ChatMessage> {
    ...
    match content.role {
        adk_core::types::Role::User => Some(ChatMessage::user(text)),
        adk_core::types::Role::Model => Some(ChatMessage::assistant(text)),
        adk_core::types::Role::System => Some(ChatMessage::system(text)),
