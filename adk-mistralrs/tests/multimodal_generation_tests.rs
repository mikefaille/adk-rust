...
proptest! {
    ...
    fn prop_role_preservation(
        role in prop_oneof![
            Just(adk_core::prelude::Role::User),
            Just(adk_core::prelude::Role::Model),
            Just(adk_core::prelude::Role::System),
        ],
        text in arb_text()
    ) {
        let content = Content {
...
fn test_empty_content_extraction() {
    let content = Content { role: adk_core::prelude::Role::User, parts: vec![] };
