...
fn make_event(author: &str, text: &str) -> Event {
    ...
    event.set_content(Content {
        role: if author == "user" { adk_core::prelude::Role::User } else { adk_core::prelude::Role::Model },
...
async fn test_summarize_produces_compaction_event() {
    ...
    assert_eq!(compaction.compacted_content.role, adk_core::prelude::Role::Model);
