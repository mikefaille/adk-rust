...
impl FunctionTool {
    ...
    where
        T: JsonSchema,
    {
        self.parameters_schema = Some(static_schema::for_deserialize::<T>().map_err(|e| {
            adk_core::AdkError::tool(format!("failed to generate parameters schema: {e}"))
        })?);
        Ok(self)
    }
    ...
    where
        T: JsonSchema,
    {
        self.response_schema = Some(static_schema::for_serialize::<T>().map_err(|e| {
            adk_core::AdkError::tool(format!("failed to generate response schema: {e}"))
        })?);
        Ok(self)
    }
...
where
    T: JsonSchema,
{
    static_schema::for_deserialize::<T>()
        .map(|doc| doc.document().clone())
        .map_err(|e| adk_core::AdkError::tool(format!("failed to generate JSON Schema: {e}")))
}
