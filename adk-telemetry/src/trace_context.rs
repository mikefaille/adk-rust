...
pub trait TraceContextExt: ReadonlyContext {
    ...
    /// Note: span.record() is removed here because the `adk_span!` macro handles it natively
    /// without allocations.
    ...
    /// (Requires that the span was initialized with these dynamic fields).
    fn record_metadata(&self, span: &Span) {
        for (key, value) in self.metadata() {
            span.record(key.as_str(), value.as_str()); // ✅ Zero allocation via as_str()
        }
    }
}
