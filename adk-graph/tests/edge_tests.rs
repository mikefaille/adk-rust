...
// ==========================================
// Domain 1: Edge Target Logic
// Tests target conversions, constants, and equality
// ==========================================

#[test]
fn test_edge_target_conversions() {
    // Prove the framework END constant maps strictly to the explicit End variant
    assert_eq!(EdgeTarget::from(END), EdgeTarget::End);
    
    // Prove the framework START constant maps strictly to a Node containing the START string
    let start: EdgeTarget = START.into();
    assert_eq!(start, EdgeTarget::Node(START.to_string()));

    // Prove arbitrary string conversions correctly map to Node variants
    let custom_node: EdgeTarget = "my_custom_node".into();
    assert_eq!(custom_node, EdgeTarget::Node("my_custom_node".to_string()));
}

#[test]
fn test_edge_target_equality() {
    // Prove identical nodes match
...
fn test_by_field_router() {
    ...
    // Standard routing based on string values
