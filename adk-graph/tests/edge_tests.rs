...
fn test_edge_target_from_str() {
    ...
    assert_eq!(end, EdgeTarget::Node("end".to_string()));
}

#[test]
fn test_edge_target_from_end_str() {
    let end: EdgeTarget = END.into();
...
fn test_by_field_router() {
    ...
    state.insert("action".to_string(), json!("save_node"));
    assert_eq!(router(&state), "save_node");

    state.insert("action".to_string(), json!("delete_node"));
